...
// ═══════════════════════════════════════════════
const EVENTS = [
  { zone:"courchevel", name:"Slopes close — après-ski", endHour:16.5, boost:2.6, repeat:"daily" },
  { zone:"meribel",    name:"Slopes close — après-ski", endHour:16.5, boost:2.4, repeat:"daily" },
  { zone:"valthorens", name:"Slopes close — après-ski", endHour:16.3, boost:2.2, repeat:"daily" },
  { zone:"courchevel", name:"Bars closing",             endHour:2.0,  boost:2.8, repeat:"fri-sat" },
  { zone:"meribel",    name:"Bars closing",             endHour:1.5,  boost:2.4, repeat:"fri-sat" },
  { zone:"moutiers",   name:"Changeover-day arrivals",  endHour:12.0, boost:3.2, repeat:"sat" },
  { zone:"moutiers",   name:"Evening train",            endHour:22.0, boost:1.8, repeat:"daily" },
];
...
const CACHE_KEY='tarn_taxi_coords_v1', CACHE_TTL=7*24*3600*1000;
...
function showAirportSchedule() {
  ...
  html+='<div style="font-weight:800;font-size:15px;margin-bottom:10px;color:var(--cyan)">✈️ Passenger exits — GVA</div>';
  ...
  if(upcoming.length===0 || cacheIsOld){
    ...
    if(cacheIsOld){
      ...
      html+=`<div style="background:rgba(2,132,199,.1);border:1px solid var(--cyan);border-radius:10px;padding:14px;text-align:center;margin-bottom:10px">
        ...
        <div style="font-size:13px;color:var(--muted);margin-top:3px">${next.fn} from ${(next.depAirport||'').slice(0,20)} ${next.nonSchengen?'🛂':'🇪🇺'}</div>
      </div>`;
...
async function loadWeather(){
  ...
  try{
    const r=await fetch(`https://api.openweathermap.org/data/2.5/weather?lat=45.4436&lon=6.6392&units=metric&lang=en&appid=${OWM_KEY}`);
    const d=await r.json();
